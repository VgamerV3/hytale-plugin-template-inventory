...
public final class InventoryStatusCommand extends AbstractCommand {
  private final InventoryPluginState state;
  private final InventoryDemoService demoService;
  ...
  public InventoryStatusCommand(
      InventoryPluginState state,
      InventoryDemoService demoService,
      AtomicLong heartbeatTicks,
      LongSupplier uptimeSeconds,
      BooleanSupplier heartbeatActive) {
    super("hdinventorystatus", "Shows runtime status for the Inventory template.");
  ...
  protected CompletableFuture<Void> execute(CommandContext ctx) {
    ...
    ctx.sendMessage(Message.raw("[Inventory] sender=" + sender + ", lastAction=" + demoService.describeLastAction(sender) + ", " + demoService.diagnostics()));
